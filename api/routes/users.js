...
const { getAllTeachers, getOneTeacher, getOneStudent, 
  toRegisterAStudent, toRegisterATeacher, verifyIfStudentExists, verifyIfTeacherExists } 
...
router.post('/login', (req, res) => {
  ...
  if (mail.match(/^[äöüéèa-zA-Z0-9]+[-_\.]*[äöüéèa-zA-Z0-9]*@student.vinci.be$/)) {
    if(!verifyIfStudentExists(mail)) return res.status(400).json("Cet utilisateur n'existe pas ou l'email est non valide")
    isUser = getOneStudent(mail, password);
  }else if (mail.match(/^[äöüéèa-zA-Z0-9]+[-_\.]*[äöüéèa-zA-Z0-9]*@vinci.be$/)){ /*   ATTENTION CHANGEMENT POUR LES TESTS */
    if(!verifyIfTeacherExists(mail))  return res.status(400).json("Cet utilisateur n'existe pas ou l'email est non valide")
  ...
  if (!bcrypt.compareSync(password, cryptedPassword)) return res.status(400).json("Le mot de passe n'est pas correct")
...
router.post('/register', (req,res) => {
  ...
  if(password !== passwordConfirm) return res.status(400).json('les mots de passe ne correspondent pas'); 
  if (!mail.match(/^[äöüéèa-zA-Z0-9]+[-_\.]*[äöüéèa-zA-Z0-9]*@student.vinci.be$/)) return res.status(400).json("Email non valide ou n'appartenant pas à un étudiant vinci");
  const encryptedPassword = bcrypt.hashSync(password, saltRounds);
  let potentialUser;
  potentialUser = toRegisterAStudent(mail, encryptedPassword);
...
router.post('/registerTeacher', (req,res) => {
   ...
   if (!mail.match(/^[äöüéèa-zA-Z0-9]+[-_\.]*[äöüéèa-zA-Z0-9]*@student.vinci.be$/)) /* gérer l'erreur */ return res.status(400).json("Email non valide ou n'appartenant pas à un professeur vinci");
